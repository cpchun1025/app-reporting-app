...
from app.models import Trade, TradeEntrySnapshot
...
def require_expected_version(trade: Trade, expected_version: int) -> None:
...
def ensure_unlocked(trade: Trade, current_user: CurrentUser) -> None:
...
def list_trades(_: CurrentUser, db: DbSession) -> list[Trade]:
...
def lock_trade(trade_id: str, current_user: CurrentUser, db: DbSession) -> Trade:
    trade = locked_trade_or_404(db, trade_id)
    ensure_unlocked(trade, current_user)
    trade.locked = True
...
def unlock_trade(trade_id: str, current_user: CurrentUser, db: DbSession) -> Trade:
    trade = locked_trade_or_404(db, trade_id)
...
@router.post("/entry/save", response_model=TradeEntrySaveResponse)
def save_trade_entry(payload: TradeEntrySaveRequest, current_user: CurrentUser, db: DbSession) -> TradeEntrySaveResponse:
    ...
    try:
        ...
        try:
            with os.fdopen(fd, "w", encoding="utf-8") as handle:
                json.dump({"business_date": payload.business_date.isoformat(), "saved_data": payload.model_dump(mode="json")}, handle, indent=2)
                handle.flush()
